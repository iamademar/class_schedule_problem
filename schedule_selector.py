...
def find_schedules(target_courses, available_courses, num_versions=3):
    schedules = []
    target_crsnos = [(course['CRSNO'], course['CLASS TYPE']) for course in target_courses]

    def build_schedule(current_schedule, remaining_courses, version):
        if not remaining_courses:
            schedules.append(current_schedule)
            return
        
        if len(schedules) >= num_versions:
            return
        ...
        for _, section in next_sections.iterrows():
            ...
            if check_gap(new_schedule) or next_course[0] in current_schedule['CRSNO'].values:
                build_schedule(new_schedule, remaining_courses[1:], version)
            elif next_course[0] not in current_schedule['CRSNO'].values:
                new_schedule = pd.concat([current_schedule, section.to_frame().T])
                build_schedule(new_schedule, remaining_courses[1:], version)
    
    for version in range(num_versions):
        build_schedule(pd.DataFrame(), target_crsnos, version)
    
    return schedules

# Find valid schedules that meet the specified criteria, allowing up to 3 versions
valid_schedules = find_schedules(target_courses, filtered_courses_df, num_versions=3)
...
if valid_schedules_with_violations:
    ...
    "No valid schedules found that include all target courses."